...
pub fn render(frame: &mut Frame, app: &mut App) {
    ...
    if app.loading {
        let msg = Paragraph::new("Computing...").block(Block::bordered().title(format!(
            " {} — '{}' [Esc to go back] ",
    ...
    let Some(df) = &app.stats_result else {
        let msg =
            Paragraph::new("No data").block(Block::bordered().title(format!(" {} ", view_label)));
        frame.render_widget(msg, area);
        return;
    };
    ...
    // Header row.
    let header = Row::new(col_names.clone()).style(
        Style::default()
            .fg(Color::Yellow)
    ...
    let rows: Vec<Row> = (0..df.height())
        .map(|ri| {
            let cells: Vec<String> = col_names
                .iter()
                .map(|cn| match df.column(cn.as_str()) {
    ...
    let constraints: Vec<Constraint> = col_names
        .iter()
        .map(|name| {
            let w = (name.len() as u16).clamp(12, 30);
            Constraint::Length(w)
        })
        .collect();

    let title = format!(" {} — '{}' [Esc to go back] ", view_label, app.stats_column);
